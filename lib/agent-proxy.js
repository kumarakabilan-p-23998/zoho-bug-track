...
/**
 * Apply a patch to a file via agent (smart merge with backup).
 */
function applyPatch(agentUrl, filePath, code, force) {
  return agentPost(agentUrl, '/apply-patch', { path: filePath, code: code, force: !!force }, 30000);
}

/**
 * Revert a file to its backup via agent.
 */
function revertFile(agentUrl, filePath) {
  return agentPost(agentUrl, '/revert-file', { path: filePath }, 15000);
}
...
/**
 * Get the component registry (rendering metadata for all components).
 * Returns { components: [{ name, tagName, classNames, classNameBindings, actions, ... }] }
 */
function getComponentRegistry(agentUrl) {
  return agentGet(agentUrl, '/component-registry', 15000);
}
