...
function analyze(apiKey, prompt, opts, callback) {
  ...
  if (images.length > 0) {
    ...
    console.log('[claude] Including', images.length, 'image(s) in multimodal prompt');
  } else {
    content = prompt;
  }
  ...
  logger.info('AI_PROMPT', 'Claude API request', { model: model, promptLength: prompt.length, imageCount: images.length, provider: 'anthropic' });
  ...
  var req = https.request(requestOptions, function (res) {
    ...
    res.on('end', function () {
      ...
      if (res.statusCode !== 200) {
        ...
        logger.error('AI_RESPONSE', 'Claude API error', { status: res.statusCode, error: errMsg, model: model, provider: 'anthropic' });
      ...
      logger.info('AI_RESPONSE', 'Claude API response', {
  ...
  req.on('error', function (err) {
    ...
    logger.error('AI_RESPONSE', 'Claude API request error', { error: err.message, model: model, provider: 'anthropic' });
