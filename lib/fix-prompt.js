...
function buildPrompt(bugDetails, analysis, extraDescription, templateContext, parsedDesc) {
  ...
  if (templateContext && templateContext.hasTemplate) {
    ...
    // ── JS Dependencies (services, mixins, helpers used by components/route) ──
    if (templateContext.dependencies) {
      var depCtx = templateContext.dependencies;
      if (depCtx.services && depCtx.services.length > 0) {
        pageLines.push('### Ember Services (injected by components/route)');
        pageLines.push('> These services are injected via `Ember.inject.service()` in the page components or route.');
        pageLines.push('');
        depCtx.services.forEach(function (svc) {
          pageLines.push('#### `' + svc.path + '`');
          pageLines.push('```js');
          pageLines.push(svc.content);
          pageLines.push('```');
          pageLines.push('');
        });
      }
      if (depCtx.mixins && depCtx.mixins.length > 0) {
        pageLines.push('### Ember Mixins (used by components/route)');
        pageLines.push('');
        depCtx.mixins.forEach(function (mixin) {
          pageLines.push('#### `' + mixin.path + '`');
          pageLines.push('```js');
          pageLines.push(mixin.content);
          pageLines.push('```');
          pageLines.push('');
        });
      }
      if (depCtx.helpers && depCtx.helpers.length > 0) {
        pageLines.push('### Template Helpers (used in page templates)');
        pageLines.push('');
        depCtx.helpers.forEach(function (helper) {
          pageLines.push('#### `' + helper.path + '`');
          pageLines.push('```js');
          pageLines.push(helper.content);
          pageLines.push('```');
          pageLines.push('');
        });
      }
  ...
  // Output format — STRICT rules for parseable code blocks
  sections.push([
    ...
    '4. 🛠 **Code Fix** — Actual code changes (FOLLOW THE CODE BLOCK RULES BELOW)',
    '5. 🧪 **Test Cases** — Manual/automated test suggestions',
    '6. 🚨 **Regression Risks** — What could break',
    '7. 📌 **Summary** — One-paragraph summary',
    '',
    '## ⚠️ CRITICAL: Code Block Rules (MUST FOLLOW)',
    '',
    'Your code blocks will be automatically parsed and applied to the codebase.',
    'Follow these rules EXACTLY or the fix will fail:',
    '',
    '### Rule 1: Every code block MUST have a `FILE:` header line immediately above it',
    '',
    'Format: **FILE: `path/to/file.ext`**',
    '',
    'Example:',
    '',
    '**FILE: `components/my-component/component.js`**',
    '```js',
    '// ... fixed code ...',
    '```',
    '',
    '### Rule 2: Only include FIXED code — never "before" code',
    '',
    '- Do NOT show the current/broken code in a code block',
    '- Do NOT show "before and after" code blocks',
    '- If you need to reference the buggy code, describe it in plain text',
    '- Only the FIXED version goes in a fenced code block',
    '',
    '### Rule 3: Include the COMPLETE fixed section — not just the changed line',
    '',
    '- Include the full function/action/computed property that was modified',
    '- Include enough surrounding context (3-5 lines above and below the change)',
    '  so the system can locate WHERE in the file to apply the fix',
    '- Do NOT include the entire file unless most of it changed',
    '- If only one function changed, include just that function with context',
    '',
    '### Rule 4: Use the REAL file path from the project',
    '',
    '- Use paths relative to the project root (e.g. `components/foo/component.js`)',
    '- Use the EXACT paths shown in the "File Contents" or "Page Files" sections above',
    '- Never use fake names like "example.js", "file.js", "component.js"',
    '',
    '### Rule 5: One code block per file, per change',
    '',
    '- If one file needs multiple changes, combine them into ONE code block',
    '  that includes all changed functions/sections with enough context',
    '- If multiple files need changes, use separate code blocks each with its own FILE: header',
    ''
  ].join('\n'));
...
function generatePrompt(userId, bugDetails, extraDescription, parsedDesc) {
  ...
  // ── Enrich keyword source with structured QA data ──
  if (parsedDesc) {
    var structuredTextL = '';
    if (parsedDesc.page) structuredTextL += ' ' + parsedDesc.page;
    if (parsedDesc.expected) structuredTextL += ' ' + parsedDesc.expected;
    if (parsedDesc.actual) structuredTextL += ' ' + parsedDesc.actual;
    if (parsedDesc.steps) {
      for (var psi2 = 0; psi2 < parsedDesc.steps.length; psi2++) {
        structuredTextL += ' ' + parsedDesc.steps[psi2];
      }
    }
    if (structuredTextL) {
      fullText += structuredTextL;
      desc += structuredTextL;
    }
  }
...
function generatePromptViaAgent(userId, bugDetails, agentUrl, extraDescription, callback, templateContext, parsedDesc) {
  ...
  // ── Enrich keyword source with structured QA data ──
  // parsedDesc.page often contains route/component names like "Settings > Connections"
  // parsedDesc.expected/actual contain specific values/identifiers related to the bug
  if (parsedDesc) {
    var structuredText = '';
    if (parsedDesc.page) structuredText += ' ' + parsedDesc.page;
    if (parsedDesc.expected) structuredText += ' ' + parsedDesc.expected;
    if (parsedDesc.actual) structuredText += ' ' + parsedDesc.actual;
    if (parsedDesc.steps) {
      for (var psi = 0; psi < parsedDesc.steps.length; psi++) {
        structuredText += ' ' + parsedDesc.steps[psi];
      }
    }
    if (structuredText) {
