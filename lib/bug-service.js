...
/**
 * Download image attachments for a bug and return as base64.
 * Filters to image types only (png, jpg, jpeg, gif, bmp, webp).
 * Limits to 3 images max and 5MB total to keep AI prompts manageable.
 * callback(err, images[]) where images = [{ name, base64, mediaType }]
 */
function downloadBugImages(userId, attachments, callback) {
  ...
  var MAX_IMAGES = 3;
  var MAX_TOTAL_BYTES = 5 * 1024 * 1024; // 5MB total

  // Filter to image attachments only
  var imageAtts = attachments.filter(function (a) {
  ...
  console.log('[bug-service] Downloading', imageAtts.length, 'image attachment(s)...');
  var results = [];
  var totalBytes = 0;
  var pending = imageAtts.length;
  var done = false;

  imageAtts.forEach(function (att, idx) {
    if (!att.uri) {
      pending--;
      if (pending === 0 && !done) { done = true; callback(null, results); }
      return;
    }

    zohoClient.zohoDownload(userId, att.uri, function (err, buffer, contentType) {
      if (done) return;
      ...
      if (!err && buffer) {
        ...
        if (totalBytes <= MAX_TOTAL_BYTES) {
          ...
          console.log('[bug-service]   ✅ Image', (idx + 1) + ':', att.name, '(' + buffer.length, 'bytes)');
        } else {
          console.log('[bug-service]   ⚠️ Skipping', att.name, '— total size exceeds 5MB limit');
        }
      } else if (err) {
        console.log('[bug-service]   ⚠️ Failed to download', att.name + ':', err.message);
      }

      pending--;
      if (pending === 0 && !done) {
        done = true;
        console.log('[bug-service] Downloaded', results.length, 'image(s),', totalBytes, 'bytes total');
...
module.exports = {
  ...
  downloadBugImages: downloadBugImages,
