...
function analyze(prompt, modelId, opts, callback) {
  ...
  // Build request body — include images if present
  var bodyObj = { prompt: prompt, modelFamily: family };
  if (images.length > 0) {
    // Append vision note to prompt text
    bodyObj.prompt = prompt + '\n\n> NOTE: ' + images.length + ' bug screenshot(s) attached. '
      + 'Analyze the visual evidence to identify the exact UI issue and its root cause in code.';
    // Pass images for the bridge extension to include in the LM request
    bodyObj.images = images.map(function (img) {
      return {
        base64: img.base64,
        mediaType: img.mediaType || 'image/png',
        name: img.name || 'screenshot'
      };
    });
    console.log('[copilot-bridge] Including', images.length, 'image(s) in request');
    logger.info('AI_PROMPT', 'Copilot Bridge including images', { imageCount: images.length, provider: 'copilot-bridge' });
  }
  var requestBody = JSON.stringify(bodyObj);

  console.log('[copilot-bridge] Request — family:', family, '(from:', modelId + ') prompt:', prompt.length, 'chars');
  logger.info('AI_PROMPT', 'Copilot Bridge request', { family: family, modelId: modelId, promptLength: prompt.length, imageCount: images.length, provider: 'copilot-bridge' });
  ...
  var req = http.request({
    ...
    res.on('end', function () {
      ...
      if (res.statusCode !== 200 || data.error) {
        var bridgeErr = data.error || ('Bridge returned HTTP ' + res.statusCode);
        logger.error('AI_RESPONSE', 'Copilot Bridge error', { status: res.statusCode, error: bridgeErr, family: family, provider: 'copilot-bridge' });
        return callback(new Error(bridgeErr));
      }
      console.log('[copilot-bridge] Response —', (data.text || '').length, 'chars, model:', data.model);
      logger.info('AI_RESPONSE', 'Copilot Bridge response', {
  ...
  req.on('error', function (err) {
    logger.error('AI_RESPONSE', 'Copilot Bridge not reachable', { error: err.message, family: family, provider: 'copilot-bridge' });
    callback(new Error('Copilot Bridge not reachable: ' + err.message));
  });

  req.on('timeout', function () {
    req.abort();
    logger.error('AI_RESPONSE', 'Copilot Bridge timeout', { timeoutMs: timeoutMs, family: family, provider: 'copilot-bridge' });
